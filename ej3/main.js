QUnit.test("ej3",function (assert) {
  var gerente = new Gerente("some","34567894-A","+52. 32503536");
    ...
    assert.ok(gerente.check("documento"), "documento ok gerente" );
    assert.ok(gerente.check("telefono"), "telefono ok gerente" );
    ...
    assert.ok(habitacion.check("telefono"), "telefono ok habitacion");
    ...
    assert.ok(hotel.check("telefono"), "telefono ok hotel" );
    assert.ok(hotel.check("sitio"), "sitio ok hotel" );
})